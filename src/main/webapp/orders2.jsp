...
<title>锡货超市</title>
<link href="ordercss/css/public.css" type="text/css" rel="stylesheet" />
<link rel="stylesheet" type="text/css" href="ordercss/css/base.css" />
<script type="text/javascript" src="ordercss/js/jquery_cart.js"></script>
<link rel="stylesheet" type="text/css" href="ordercss/css/checkOut.css" />
<script src="js/jquery-1.9.1.min.js"></script>
<script type="text/javascript">

	
function sendorder(){
	var count =${sessionScope.countone};
	var ids="${sessionScope.idss}";
	var user_id=${sessionScope.user.id};
	var amount=${sessionScope.allcount2};
    var status=0;
    var assessstatus=0;
    var comments=$(".comments").val();
	var nowamount=$(".allcount").text();
	var price2=${sessionScope.price2};
	var nowprice2=${sessionScope.nowprice2};
	var address_id=0;

	   $(".addrid").each(function(){
			   if($(this).siblings(".chk").prop("checked")){ 
				   address_id=$(this).attr("myid");
			   }
			});
	 ...
	 $.ajax({
		    url: "sendorder",
	        type: "POST",
	        data: {nowprice2:nowprice2,price2:price2,count2:count,user_id:user_id,address_id:address_id,assessstatus:assessstatus,comments:comments,status:status,productid:ids,nowamount:nowamount,amount:amount},
...
function editaddr(){ 
	var user_id=${sessionScope.user.id};
	var name=$(".input1").val();
    var myselect=document.getElementById("Provinces");
    var index=myselect.selectedIndex ;
    var zone1=myselect.options[index].text;
    var myselect=document.getElementById("Citys");
    var index=myselect.selectedIndex ;
    var zone2=myselect.options[index].text;
    var myselect=document.getElementById("Countys");
    var index=myselect.selectedIndex ;
    var zone3=myselect.options[index].text;
    var status=0;
    var addr=$(".input-area").val();
    var tel=$(".input2").val();
      
       $.ajax({
		    url: "editaddr",
	        type: "POST",
	        data: {"user_id":user_id,"zone1":zone1,"zone2":zone2,"zone3":zone3,"addr":addr,"name":name,"tel":tel,"status":status},
	         success: function (data) {
	        	if(data>0){
	        	alert("新地址添加成功！");
...
<body>
	...
	<div class="border_top_cart">
								...
								<div class="box-bd">
									<div class="clearfix xm-address-list" id="checkoutAddrList">
										...
										<c:forEach items="${requestScope.addresslist}" var="a">
											...
											<dl class="item">
											<input name="a" type="radio" class="chk" checked="checked" />
												<dt class="addrid" myid="${a.id}">
													<strong class="itemConsignee">${a.name}</strong>
												</dt>
												<dd>
													<p class="tel itemTel">${a.tel}</p>
													<p class="itemRegion">${a.zone}</p>
													<p class="itemStreet">${a.addr}</p>
													<span class="edit-btn J_editAddr" onclick="">编辑</span>
												</dd>
									...
									<input type="hidden" name="newAddress[city]" id="newCity" /> <input
									...
									<div class="xm-edit-addr-box" id="J_editAddrBox">
										<div class="bd">
											...
											<div class="item">
												<label>地址<span>*</span></label> <select
													name="userAddress[province]" id="Provinces"
													class="select-1">
												...
												<textarea name="addr" class="input-area" id="Street"
										...
										<div class="ft clearfix">
											...
											<button type="button" class="btn btn-primary  btn-small "
												onclick="editaddr();" id="J_editAddrOk">保存</button>
										</div>
									</div>
									<!--点击弹出新增/收货地址界面end-->
							...
							<div class="xm-box">
								...
								<div class="box-bd">
									<ul class="checkout-option-list clearfix J_optionList">
										<li class="item selected"><input type="radio"
											checked="checked" name="Checkout[best_time]" value="1"><p>
													不限送货时间<span>周一至周日</span>
												</p></li>
										<li class="item "><input type="radio"
											name="Checkout[best_time]" value="2"><p>
													工作日送货<span>周一至周五</span>
												</p></li>
										<li class="item "><input type="radio"
											name="Checkout[best_time]" value="3"><p>
										...
										<c:forEach items="${requestScope.orderslist}" var="o">
											<dd class="item clearfix">
												<div class="item-row">
													...
													<div class="col col-2 price" style="display: none;">${o.price}</div>
													<div class="col col-2 aaa">${o.nowprice}</div>
													<div class="col col-3">${sessionScope.countone}</div>
													<div class="col col-4">${o.nowprice*sessionScope.countone}</div>
												</div>
											</dd>
										</c:forEach>
									...
									<div class="checkout-count clearfix">
										<div class="checkout-count-extend xm-add-buy">
											<h3 class="title">
												...
												</br> <input type="text" class="comments" />
										</div>
										<!-- checkout-count-extend -->
										<div class="checkout-price">
											<p class="checkout-total">
												应付总额：<span><strong id="totalPrice" class="allcount">${o.nowprice*sessionScope.count}</strong>元</span>
											</p>
										</div>
										<!--  -->
									</div>
						...
						<input type="hidden" id="couponType" name="Checkout[couponsType]">
							...
							name="Checkout[couponsValue]">
								<div class="checkout-confirm">
									...
									<a href="shop?id=${sessionScope.user.id}"
										class="btn btn-lineDakeLight btn-back-cart">返回购物车</a> <input
										type="button" onclick="sendorder();" class="btn btn-primary"
										value="立即下单" id="checkoutToPay" />
								</div>
					</div>
			</div>
		...
		<script src="ordercss/js/base.min.js"></script>

		<script type="text/javascript" src="ordercss/js/address_all.js"></script>
		<script type="text/javascript" src="ordercss/js/checkout.min.js"></script>
	</div>
	...
	<script src="ordercss/js/unslider.min.js" type="text/javascript"></script>
	<script src="ordercss/js/index.js" type="text/javascript"></script>
